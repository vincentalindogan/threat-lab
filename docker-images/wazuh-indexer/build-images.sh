...
build() {
    ...
    #docker compose -f docker-images/build-images.yml --env-file .env build --no-cache || clean 1
    #docker compose -f build-images.yml --env-file .env build --no-cache || clean 1
    export $(cat .env | xargs) && nerdctl build \
        --build-arg WAZUH_VERSION=$WAZUH_VERSION \
        --build-arg WAZUH_IMAGE_VERSION=$WAZUH_IMAGE_VERSION \
        ...
        --no-cache -t wazuh/indexer:latest .
